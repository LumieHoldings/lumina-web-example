...
import { PSWAP_PRIVATE_MASM } from "@/lib/masm/private-pswap";
...
export default function PrivatePswapPage() {
  ...
  const runTest = useCallback(async () => {
    ...
    try {
      ...
      const createPswapNote = ({
        ...
        const pswapTag = WebClient.buildSwapTag(
          noteType,
          offeredFaucetId,
          offeredAmount,
          requestedFaucetId,
        ...
        const inputs = new NoteInputs(
          new MidenArrays.FeltArray([
            ...
            new Felt(BigInt(pswapTag.asU32())), // 4
        ...
        const metadata = new NoteMetadata(lastConsumerId, noteType, pswapTag);
      ...
      const storeName = `private-pswap-${Date.now()}`;
      log(`RPC URL: ${rpcUrl}`);
      log(`Store: ${storeName}`);
      ...
      const pswapCodeWithInjectedRoot = injectP2idRoot(
        PSWAP_PRIVATE_MASM,
        p2idRoot,
      );
      ...
      const pswapNote = createPswapNote({
      ...
      const pswapNoteId = pswapNote.id().toString();
      setState((prev) => ({ ...prev, pswapNoteId: pswapNoteId }));
      log("");
      log("=== PSWAP NOTE CREATED ===");
      log(`  Note ID: ${pswapNoteId}`);
      log(`  Tag: ${pswapNote.metadata().tag().asU32()}`);

      await client.addTag(String(pswapNote.metadata().tag().asU32()));
      await client.addTag(
      ...
      const createReq = new TransactionRequestBuilder()
        .withOwnOutputNotes(
          new MidenArrays.OutputNoteArray([OutputNote.full(pswapNote)]),
        )
        .build();
      await submitAndApply(
        toAccountId(makerIdHex),
        createReq,
        "  PSWAP transaction submitted",
      );

      log("");
      log("--- Waiting for PSWAP note to be consumable ---");
      let foundPswap = false;
      for (let attempt = 1; attempt <= MAX_POLL_ATTEMPTS; attempt++) {
        await client.syncState();
        const consumable = await client.getConsumableNotes();
        const found = consumable.find(
          (n: any) => n.inputNoteRecord().id().toString() === pswapNoteId,
        );
        if (found) {
      ...
      if (!foundPswap) {
        throw new Error(
          `PSWAP note not consumable after ${MAX_POLL_ATTEMPTS} attempts`,
        );
      }
      ...
      log(`  Leftover offered:   ${leftoverOffered} GOLD (in new PSWAP)`);
      log(`  Leftover requested: ${leftoverRequested} SILVER (in new PSWAP)`);
      ...
      const noteAndArgs = new NoteAndArgs(
        Note.deserialize(pswapNote.serialize()),
        noteArgs,
      );
      ...
      log(
        `  Leftover PSWAP: ${leftoverOffered} GOLD (note ${expectedLeftoverId})`,
      );
      log("");
      log("Done.");
      setPhase("done");
  ...
  return (
    ...
    >
      <div style={{ maxWidth: "1000px", margin: "0 auto" }}>
        ...
        {(state.goldFaucetId || state.makerId || state.pswapNoteId) && (
          <div
            style={{
              marginBottom: "20px",
              padding: "14px",
          ...
          >
            ...
            <div>PSWAP Note: {state.pswapNoteId ?? "-"}</div>
        ...
        >
          ...
          {state.logs.length === 0 ? (
            <p style={{ color: "#6b7280" }}>
              Click &quot;Run PSWAP Advanced&quot; to start.
            </p>
          ) : (
